...
目前 Viter 后端集成完成继承自 Vite，因此后端集成方案可以完全参照[ Vite 后端集成章节](https://cn.vitejs.dev/guide/backend-integration.html)。
...
1. 在你的 Viter 配置中配置入口文件和启用创建 `manifest`：
   ...
   如果你没有禁用 [module preload 的 polyfill](https://cn.vitejs.dev/config/#build-polyfillmodulepreload)，你还需在你的入口处添加此 polyfill：
...
3. 在生产环境中：在运行 `viter build` 之后，一个 `manifest.json` 文件将与静态资源文件一同生成。一个示例清单文件会像下面这样：
